...
class GraphWin(tk.Canvas):
    ...
    def checkKey(self):
        """Return last key pressed or None if no key pressed since last 
call"""
        if self.isClosed():
...
class Circle(Oval):
    ...
    def __repr__(self):
        return "Circle({}, {})".format(str(self.getCenter()), 
str(self.radius))
...
class Image(GraphicsObject):
    ...
    def __init__(self, p, *pixmap):
        ...
        if len(pixmap) == 1: # file name provided
            ...
            width, height = pixmap
            self.img = tk.PhotoImage(master=_root, width=width, 
height=height)
...
self.getHeight())
    ...
    def getPixel(self, x, y):
        """Returns a list [r,g,b] with the RGB color values for pixel 
(x,y)
    ...
    def save(self, filename):
        ...
        The format for the save image is determined from the filname 
extension.
...
class ShapeManager:
    def __init__(self, window_title, window_width, window_height):
        # Create the GraphWin object to display the shapes
    ...
    def add_rectangle(self, bottom_left, top_right, color):
        # Create a rectangle with the given attributes
    ...
    def change_color(self, index, color):
        # Change the color of the shape at the given index
        shape = self.shapes[index]
        shape.setFill(color)
        
    def delete_shape(self, index):
        # Remove the shape from the window and the list
        shape = self.shapes.pop(index)
        shape.undraw()
        
# Create the ShapeManager object
shape_manager = ShapeManager("Shape Manager", 400, 400)

# Add shapes to the window
shape_manager.add_circle((2, 2), 2, "red")  # Red circle at (2, 2) with radius 2
shape_manager.add_rectangle((3, 3), (6, 5), "blue")  # Blue rectangle from (3, 3) to (6, 5)
shape_manager.add_circle((2, 2), 1, "green")  # Green circle at (2, 2) with radius 1
